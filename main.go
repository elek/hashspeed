...
func main() {
	...
	writes := flag.Int("writes", 1024, "Bytes used in hash.Write in one step")
	size := flag.Int("size", 657743, "Calculate the hash after this amount of bytes were written.")
	algo := flag.String("algo", "sha256", "Hash algorithm to be used ("+keys(algos)+")")
	flag.Parse()
